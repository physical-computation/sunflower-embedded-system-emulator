...
void putseq()
{
  ...
  for (i=0; i<nframes; i++)
  {
    ...
    else
    {
       ...
       */
       fprintf(statfile,"\n motion estimation started\n");
      motion_estimation(oldorgframe[0],neworgframe[0],
                        oldrefframe[0],newrefframe[0],
                        neworg[0],newref[0],
                        sxf,syf,sxb,syb,mbinfo,0,0);

      fprintf(statfile,"\n motion estimation done\n");

      predict(oldrefframe,newrefframe,predframe,0,mbinfo);

       fprintf(statfile,"\n motion prediction done\n");

      dct_type_estimation(predframe[0],neworg[0],mbinfo);
      transform(predframe,neworg,mbinfo,blocks);

       fprintf(statfile,"\n motion transform done\n");
      ...
      fprintf(statfile,"all done for the frame\n"); 
    }

    //sprintf(name,tplref,f+frame0);
    //writeframe(name,newref);

    fprintf(statfile,"Encoding of frame %d done !\n",i);
