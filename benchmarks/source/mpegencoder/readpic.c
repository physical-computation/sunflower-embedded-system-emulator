...
// read file kodified by Koushik for testing
...
unsigned char *frame[];
{
  ...
  for (i=0; i<vertical_size; i++)
  {
    for(j=0;j<horizontal_size;j++) 
      {
	*(frame[0]+i*width+j)=(unsigned char)(rand()%255);
  ...
  for (i=0; i<chrom_vsize; i++)
    {
      for(j=0;j<chrom_hsize;j++) 
	{
	  *(frame[1]+i*chrom_width+j)=(unsigned char)(rand()%255);
  ...
  for (i=0; i<chrom_vsize; i++)
    {
      for(j=0;j<chrom_hsize;j++) 
	{
	 *(frame[2]+i*chrom_width+j)=(unsigned char)(rand()%255);
