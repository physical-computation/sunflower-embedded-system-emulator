...
with fileinput.input(inplace=True) as file:
    for line in file:
        lineno = file.lineno()
        replaced = replace_mnemonic(line, "fadd.s")
        replaced = replace_mnemonic(line, "fsub.s")
        replaced = replace_mnemonic(line, "fmul.s")
        replaced = replace_mnemonic(line, "fdiv.s")
        replaced = replace_mnemonic(line, "fsqrt.s")
        replaced = replace_mnemonic(line, "fsgnj.s")
        replaced = replace_mnemonic(line, "fsgnjn.s")
        replaced = replace_mnemonic(line, "fssnjx.s")
        # psuedo instructions
        replaced = replace_mnemonic(line, "fmv.s")
        replaced = replace_mnemonic(line, "fneg.s")
        ...
        if re.search(r"^\tflw", replaced) != None:
            eprint(
...
Error: this should have been replaced"
        ...
        if re.search(r"^\tfsw", replaced) != None:
            eprint(
