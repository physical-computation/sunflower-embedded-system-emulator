...
static phmm_t *phmm_lookup (s3pid_t pid)
{
    ...
    for (p = ci_phmm[mdef->phone[pid].ci]; p; p = p->next) {
	old = &(mdef->phone[p->pid]);
	if (old->tmat == new->tmat) {
	    for (s = 0; (s < mdef->n_emit_state) && (old->state[s] == new->state[s]); s++);
	    if (s == mdef->n_emit_state)
		return p;
	}
    }
...
static int32 phmm_build ( void )
{
	...
	for (p = ci_phmm[ci]; p; p = p->next) {
		...
		p->sen[s] = mdef->phone[p->pid].state[s];
	...
	if (IS_CIPID(mdef->phone[pid].lc) && mdef->ciphone[mdef->phone[pid].lc].filler) {
	    for (i = 0; IS_CIPID(filler[i]); i++)
		lrc_set (p->lc, filler[i]);
	} else
	    lrc_set (p->lc, mdef->phone[pid].lc);
	
	if (IS_CIPID(mdef->phone[pid].rc) && mdef->ciphone[mdef->phone[pid].rc].filler) {
	    for (i = 0; IS_CIPID(filler[i]); i++)
...
int32 allphone_start_utt (char *uttid)
{
	...
	for (p = ci_phmm[ci]; p; p = p->next) {
	    p->active = -1;
	    p->inscore = LOGPROB_ZERO;
	    p->bestscore = LOGPROB_ZERO;

	    for (s = 0; s <= mdef->n_emit_state; s++) {
		p->score[s] = LOGPROB_ZERO;
		p->hist[s] = NULL;
	    }
	}
    }
    ...
    ci = mdef_ciphone_id (mdef, SILENCE_CIPHONE);
    if (NOT_CIPID(ci))
	E_FATAL("Cannot find CI-phone %s\n", SILENCE_CIPHONE);
    for (p = ci_phmm[ci]; p && (p->pid != ci); p = p->next);
    if (! p)
...
static void phmm_eval (phmm_t *p, int32 *senscr)
{
	...
	bestfrom = LOGPROB_ZERO;
	for (from = to; from >= 0; from--) {
	    if ((tp[from][to] > LOGPROB_ZERO) && (p->score[from] > LOGPROB_ZERO)) {
...
static void phmm_trans ( void )
{
	...
	for (l = from->succlist; l; l = l->next) {
	    ...
	    newscore = h->score + tp[from->ci][to->ci];
...
{
    int32 f, sf, score, latbeam, best, thresh, nnode;
    history_t *h;
    char filename[4096];
    FILE *fp;
    float64 *f64arg;
...
static void phone_tp_init (char *file, float64 floor, float64 wt, float64 ip)
{
	...
	tp[pid1][pid2] = (int32)(logs3(p) * wt) + inspen;
    }
