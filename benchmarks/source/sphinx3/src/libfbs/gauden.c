			       ...
			       int32 ** out_veclen, const char *file_name)
{
	...
	/* #Codebooks */
	if (bio_fread(&n_mgau, sizeof(int32), 1, fp, byteswap, &chksum) !=
	    1)
		E_FATAL("fread(%s) (#codebooks) failed\n", file_name);
	*out_n_mgau = n_mgau;

	/* #Features/codebook */
	if (bio_fread(&n_feat, sizeof(int32), 1, fp, byteswap, &chksum) !=
	    1)
	...
	*out_veclen = veclen;
	...
	if (!(*out_param)) {
		out =
		    (vector_t ***) ckd_calloc_3d(n_mgau, n_feat, n_density,
						 sizeof(vector_t));
...
static int32 gauden_dist_precompute(gauden_t * g, float32 varfloor)
{
	...
	for (m = 0; m < g->n_mgau; m++) {
		for (f = 0; f < g->n_feat; f++) {
			...
			for (d = 0, detp = g->det[m][f]; d < g->n_density;
			     d++, detp++) {
				*detp = (float32) 0.0;

				for (i = 0, varp = g->var[m][f][d];
				     i < flen; i++, varp++) {
					if (*varp < varfloor)
						*varp = varfloor;
...
gauden_t *gauden_init(char *meanfile, char *varfile, float32 varfloor)
{
	...
	gauden_param_read(&(g->mean), &g->n_mgau, &g->n_feat,
			  &g->n_density, &g->featlen, meanfile);
	gauden_param_read(&(g->var), &m, &f, &d, &flen, varfile);
	// Added by Fang Fang for debugging 
	/*
	   mean_fp = fopen("mean_file.txt", "w");
	   var_fp  = fopen("var_file.txt", "w");
	   for (fi = 0 ; fi < g->n_mgau ; fi++){
	   for (fj = 0; fj < g->n_feat; fj++){
	   for (fk = 0; fk < g->n_density; fk++){
	   for (fs = 0; fs < 39; fs ++){
	   fprintf(mean_fp, "%f ", (*g->mean)[fi][fj][fk*39+fs]);
	   fprintf(var_fp, "%f ", (*g->var)[fi][fj][fk*39+fs]);
	   }
	   }
	   fprintf(mean_fp, "\n");
	   fprintf(var_fp, "\n");
...
int32 gauden_mean_reload(gauden_t * g, char *meanfile)
{
	...
	for (i = 0; i < g->n_feat; i++)
		if (g->featlen[i] != flen[i])
			E_FATAL
			    ("Feature lengths for original and new means differ\n");
	ckd_free(flen);
	     ...
	     int32 n_density)
{
	...
	for (i = 0; i < n_top; i++)
		out_dist[i].dist = MAX_IEEE_NORM_POS_FLOAT64;
	...
	for (d = 0; d < n_density; d++) {
		...
		for (i = 0; (i < n_top) && (dval >= out_dist[i].dist);
		     i++);
		assert(i < n_top);
		for (j = n_top - 1; j > i; --j)
	    ...
	    int32 n_top, vector_t * obs, gauden_dist_t ** out_dist)
{
	...
	for (f = 0; f < g->n_feat; f++) { // Comment added by fang: For s1_39x, n_feat = 1
		compute_dist(dist, n_top,
			     obs[f], g->featlen[f],
			     g->mean[mgau][f], g->var[mgau][f],
			     g->det[mgau][f], g->n_density);
	   // When it returns, dist has the top N smallest distances.  		     
		...
		for (t = 0; t < n_top; t++) {
			...
			// This is where distance in float64 is converted to distance in int32
		}
	}
