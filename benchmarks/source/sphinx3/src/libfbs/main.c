 ...
 * 
 * $Log: main.c,v $
 * Revision 1.3  2003/01/29 02:29:07  ffang
 * Add in the code to extract profiling information
 *
 * Revision 1.2  2003/01/07 04:22:16  ffang
...
static float senonefloor = 1.0;
static int senonewidth = 32; 

void senonefloor_init(float v){
	senonefloor = v;
}
void senonewidth_init(int v){
	senonewidth = v;
}	
/*
...
static arg_def_t defn[] = {
	{"-tmatwidth",
	 ...
	 "0",
	 "# of bits for transition matrices"}, 
	{"-senonefloor",
	 CMD_LN_FLOAT32,
	 CMD_LN_NO_VALIDATION,
	 "1.0",
	 "fraction to apply senone floor"},
	{"-senonewidth",
	 CMD_LN_FLOAT32,
	 CMD_LN_NO_VALIDATION,
	 "32",
	 "# of bits for senone scores"}, 
	{"-bitwidth_ugprob",
	 CMD_LN_FLOAT32,
	 CMD_LN_NO_VALIDATION,
	 "32.0",
	 "# of bits for unigram probability"},
	 {"-bitwidth_ugbowt",
	 CMD_LN_FLOAT32,
	 CMD_LN_NO_VALIDATION,
	 "32.0",
	 "# of bits for unigram backoff weight"},
	 {"-bitwidth_bgprob",
	 CMD_LN_FLOAT32,
	 CMD_LN_NO_VALIDATION,
	 "32.0",
	 "# of bits for bigram probability"},
	 {"-bitwidth_tgprob",
	 CMD_LN_FLOAT32,
	 CMD_LN_NO_VALIDATION,
	 "32.0",
	 "# of bits for trigram probability"},
	 {"-bitwidth_tgbowt",
	 CMD_LN_FLOAT32,
	 CMD_LN_NO_VALIDATION,
	 "32.0",
	 "# of bits for trigram backoff weight"},
...
static void models_init(void)
{
	...
	if ((dict->filler_start > dict->filler_end)
	    || (!dict_filler_word(silwid)))
	...
	g = gauden_init((char *) cmd_ln_access("-meanfn"),
			(char *) cmd_ln_access("-varfn"), varfloor);
	...
	if (mdef->n_sen != sen->n_sen)
		E_FATAL
		    ("Model definition has %d senones; but #senone= %d\n",
	...
	if ((arg = (char *) cmd_ln_access("-lambdafn")) != NULL) {
		...
		if (interp->n_sen != sen->n_sen)
			E_FATAL
			    ("Interpolation file has %d weights; but #senone= %d\n",
	...
	if (mdef->n_emit_state != tmat->n_state - 1)
		E_FATAL
		    ("#Emitting states in model definition = %d, #states in tmat = %d\n",
...
{				/* In: LM score scale-factor (in dagsearch) */
	...
	fprintf(fp, "%s S %d T %d A %d L %d", uttid, scl, tscr, ascr,
		lscr);
	...
	if (!hypptr)		/* HACK!! */
		...
		for (h = hypptr; h; h = h->next) {
			lscr =
			    (dict_basewid(h->wid) !=
			     startwid) ? lm_rawscore(h->lscr, lwf) : 0;
...
static void
log_hypstr(FILE * fp, hyp_t * hypptr, char *uttid, int32 exact, int32 scr)
{
	hyp_t *h;
	s3wid_t w;
...
static void
log_hyp_detailed(FILE * fp, hyp_t * hypptr, char *uttid, char *LBL,
		 char *lbl)
{
	hyp_t *h;
...
{				/* In: Utterance id, for logging and other use */
	...
	if (!feat) {
		...
		if (topn > g->n_density) {
			E_WARN
			    ("-topn argument (%d) > #density codewords (%d); set to latter\n",
			     topn, g->n_density);
			topn = g->n_density;
		}
		dist =
		    (gauden_dist_t **) ckd_calloc_2d(n_feat, topn,
						     sizeof
						     (gauden_dist_t));
		...
		if (inlatdir) {
			E_INFO
			    ("Computing only active codebooks and senones each frame\n");
			sen_active =
			    (int8 *) ckd_calloc(sen->n_sen, sizeof(int8));
			mgau_active =
			    (int8 *) ckd_calloc(g->n_mgau, sizeof(int8));

			//Space for senone scores (one frame)
			senscr =
			    (int32 **) ckd_calloc_2d(1, sen->n_sen,
						     sizeof(int32));
		} else*/
		 {
			E_INFO
			    ("Computing all codebooks and senones each frame\n");
			sen_active = NULL;
			mgau_active = NULL;

			/* Space for senone scores (window of frames) */
			senscr =
			    (int32 **) ckd_calloc_2d(GAUDEN_EVAL_WINDOW,
						     sen->n_sen,
						     sizeof(int32));
		}
		...
		mgau2sen =
		    (mgau2sen_t **) ckd_calloc(g->n_mgau,
					       sizeof(mgau2sen_t *));
		for (s = 0; s < sen->n_sen; s++) {
			m2s =
			    (mgau2sen_t *)
			    listelem_alloc(sizeof(mgau2sen_t));
			m2s->sen = s;
			m2s->next = mgau2sen[sen->mgau[s]];
			mgau2sen[sen->mgau[s]] = m2s;
		}
		// Comment added by Fang Fang: actually here sen and mgau are already 1-to-1 correspondent 
	}
...
double rand_normal(double m, double sd){
  ...
  static bool have_extra_normal = false;
  static double extra_normal;

  if (have_extra_normal) {
    have_extra_normal = false;
    return extra_normal *sd + m;
  }
  double v1, v2, r;

  do {
    v1 = (double(rand()) / RAND_MAX ) * 2 - 1; // (-1, 1)
    v2 = (double(rand()) / RAND_MAX ) * 2 - 1; // (-1, 1)
    r = v1*v1+v2*v2;
  } while (r >= 1.0);

  double fac = sqrt(-2*log(r)/r);
  have_extra_normal = true;
  extra_normal = v1*fac ;
  return v2*fac *sd + m;
}
g, gid, topn, feat,
						    dist);
					for (m2s = mgau2sen[gid]; m2s;
					     m2s = m2s->next) {
						s = m2s->sen;
						if (sen_active[s]) {
							senscr[0][s] =
			...
			if (interp) {
				for (s = mdef->n_ci_sen; s < sen->n_sen;
				     s++) {
					if (sen_active[s])
	...
	
	{
		...
		for (j = 0; j < nfr; j += GAUDEN_EVAL_WINDOW) { //GAUDEN_EVAL_WINDOW = 8
		/* Compute Gaussian densities and senone scores for window of frames */
			...
			for (i = j, k = 0;
			     (k < GAUDEN_EVAL_WINDOW) && (i < nfr); i++, k++) {
			...
			//	for (s = 0; s < sen->n_sen; s++){ 
			//		senscr[k][s]-=best;
				//	fprintf(sen_fp, "%d ", senscr[k][s]);
			//	}
		//		fprintf(sen_fp, "\n");
				
				// added by Fang
							
				quant = (int)(log10(fabs(worst * senonefloor - best))/log10(2)+1);
				quant -= senonewidth;
				if(quant < 0) quant = 0;
			
				quant = 19 - senonewidth; 
				if(quant < 0) quant = 0;	
				
				for (s = 0; s < sen->n_sen; s++){
					// modified by Fang
						senscr[k][s] -= best;  // original line 
						if (senscr[k][s] > -pow(2.0, 19) ){ // 19 bit  
							if (quant > 0) 
								senscr[k][s] = ((senscr[k][s] + (1 << (quant-1)) ) >> quant ) << quant;
						}	
						else{ 
							senscr[k][s] = worst * 0.5 - best;
...
static void decode_utt(float32 ** mfc, int32 nfr, char *uttid)
{
	...
	if (hyp != NULL) {
		...
		for (i = 0; i < nfr; i++)
			scl += senscale[i]; //senscale:globle var -- ALL senone scores scaled by senscale[i] in frame i 
		...
		if ((outlatdir || bp) && (dag_build() == 0)) {
			...
			if (bp) {
				...
				if (*
				    ((int32 *)
				     cmd_ln_access("-backtrace")))
					log_hyp_detailed(stdout, hyp,
							 uttid, "BP",
							 "bp");
				...
				printf("BSTPTH: ");
				log_hypstr(stdout, hyp, uttid, matchexact,
					   ascr + lscr);

				printf("BSTXCT: ");
				log_hypseg(uttid, stdout, hyp, nfr, scl,
					   lwf);
			}
	...
	if (nfr > 0) {
		...
		if (tm_utt->t_cpu > 0.0) {
			printf(" [fwd %6.2fx %3d%%]",
			       tm_fwdvit->t_cpu * 100.0 / nfr,
			       (int32) ((tm_fwdvit->t_cpu * 100.0) /
					tm_utt->t_cpu));
			printf("[gau+sen %6.2fx %2d%%]",
			       tm_gausen->t_cpu * 100.0 / nfr,
			       (int32) ((tm_gausen->t_cpu * 100.0) /
					tm_utt->t_cpu));
			printf("[srch %6.2fx %2d%%]",
			       tm_fwdsrch->t_cpu * 100.0 / nfr,
			       (int32) ((tm_fwdsrch->t_cpu * 100.0) /
					tm_utt->t_cpu));

			if (bp)
				printf("[bp %6.2fx %2d%%]",
				       tm_bstpth->t_cpu * 100.0 / nfr,
				       (int32) ((tm_bstpth->t_cpu *
						 100.0) / tm_utt->t_cpu));
		}
	}
	printf("\n");
...
/* END CHANGES, 6 JAN 1998, BHIKSHA */
	...
	if (ctloffset > 0)
		E_INFO
		    ("Skipping %d utterances in the beginning of control file\n",
		     ctloffset);
	while ((ctloffset > 0)
	       && (fgets(line, sizeof(line), ctlfp) != NULL)) {
	...
	while ((ctlcount > 0)
		...
		ef = (int32) 0x7ffffff0;
		if ((k =
		     sscanf(line, "%s %d %d %s", ctlspec, &sf, &ef,
			    uttid)) <= 0)
			continue;	/* Empty line */
		...
		if (k < 4) {
			...
			for (i = strlen(ctlspec) - 1;
			     (i >= 0) && (ctlspec[i] != '/'); --i);
			if (k == 3)
				sprintf(uttid, "%s_%d_%d", ctlspec + i + 1,
					sf, ef);
			else
		...
		if (mllrctlfp) {
			...
			if (strcmp(prevmllr, mllrfile) != 0) {
				...
				gauden_mean_reload(g,
						   (char *)
						   cmd_ln_access
						   ("-meanfn"));

				if (mllr_read_regmat
				    (mllrfile, &A, &B, featlen,
				     n_feat) < 0)
					E_FATAL
					    ("mllr_read_regmat failed\n");

				mgau_xform =
				    (uint8 *) ckd_calloc(g->n_mgau,
							 sizeof(uint8));
		...
		if (ctlspec[0] != '/')
			sprintf(cepfile, "%s/%s.%s", cepdir, ctlspec,
				cepext);
		else
...
/* END CHANGES BY BHIKSHA */
			...
			if (matchfp)
				log_hypstr(matchfp, NULL, uttid,
					   matchexact, 0);
			if (matchsegfp)
				log_hypseg(uttid, matchsegfp, NULL, 0, 0,
					   1.0);
	...
	while (fgets(line, sizeof(line), ctlfp) != NULL) {
		if (sscanf(line, "%s", ctlspec) > 0) {
			E_INFO
			    ("Skipping rest of control file beginning with:\n\t%s",
			     line);
			break;
		}
	}
...
//	sen_fp = fopen("senone.txt", "w");
	...
	if (str) {
		...
		if ((argc = load_argfile(str, argv[0], &argv)) < 0) {
			...
			fprintf(stderr, "\t%s argument-list, or\n",
				argv[0]);
			fprintf(stderr,
				"\t%s [argument-file] (default file: s3decode.arg)\n\n",
				argv[0]);
			cmd_ln_print_definitions();
			fprintf(stderr, "before second exit\n");
			exit(1);
		}
	}
	...
	if ((cmd_ln_access("-mdeffn") == NULL) ||
		...
		E_FATAL
		    ("Missing -mdeffn, -meanfn, -varfn, -mixwfn, or -tmatfn argument\n");
	...
	if (inlatdir && outlatdir && (strcmp(inlatdir, outlatdir) == 0))
		E_FATAL
		    ("Input and output lattice directories are the same\n");
	...
	if(cmd_ln_access("-tmatwidth") == NULL)
		tmatwidth_init(0);
	else tmatwidth_init((int) *((float32 *) cmd_ln_access("-tmatwidth")));

	
	if(cmd_ln_access("-senonefloor") == NULL)
		senonefloor_init(1);
	else senonefloor_init((float) *((float32 *) cmd_ln_access("-senonefloor")));

	if(cmd_ln_access("-senonewidth") == NULL)
		senonewidth_init(32);
	else senonewidth_init((int) *((float32 *) cmd_ln_access("-senonewidth")));

	if(cmd_ln_access("-bitwidth_ugprob") == NULL)
		bitwidth_ugprob_init(32);
	else {
		bitwidth_ugprob_init((int) *((float32 *) cmd_ln_access("-bitwidth_ugprob")));
	}
	
	if(cmd_ln_access("-bitwidth_ugbowt") == NULL)
		bitwidth_ugbowt_init(32);
	else {
		bitwidth_ugbowt_init((int) *((float32 *) cmd_ln_access("-bitwidth_ugbowt")));
	}
	
	if(cmd_ln_access("-bitwidth_bgprob") == NULL)
		bitwidth_bgprob_init(32);
	else {
		bitwidth_bgprob_init((int) *((float32 *) cmd_ln_access("-bitwidth_bgprob")));
	}
	
	if(cmd_ln_access("-bitwidth_tgprob") == NULL)
		bitwidth_tgprob_init(32);
	else {
		bitwidth_tgprob_init((int) *((float32 *) cmd_ln_access("-bitwidth_tgprob")));
	}
	
	if(cmd_ln_access("-bitwidth_tgbowt") == NULL)
		bitwidth_tgbowt_init(32);
	else {
		bitwidth_tgbowt_init((int) *((float32 *) cmd_ln_access("-bitwidth_tgbowt")));
	}
	// End
...
/* END CHANGES BY BHIKSHA */
	...
	fprintf(stderr, "before last exit\n");
	exit(err_status);
}
