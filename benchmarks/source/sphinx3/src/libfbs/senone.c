...
static int32 senone_mgau_map_read(senone_t * s, char *file_name)
{
	...
	if (n_gauden_present) {
		if (bio_fread
		    (&(s->n_gauden), sizeof(int32), 1, fp, byteswap,
		     &chksum)
		    != 1)
...
static int32 senone_mixw_read(senone_t * s, char *file_name)
{
	...
	if ((bio_fread
	     (&(s->n_sen), sizeof(int32), 1, fp, byteswap, &chksum) != 1)
	    ||
	    (bio_fread
	     (&(s->n_feat), sizeof(int32), 1, fp, byteswap, &chksum)
	     != 1)
	    ||
	...
	E_INFO
	    ("Truncating senone logs3(pdf) values by %d bits, to 8 bits\n",
	     s->shift);
	...
	if (s->n_gauden > 1) {
		s->pdf =
		    (senprob_t ***) ckd_calloc_3d(s->n_sen, s->n_feat,
						  s->n_cw,
						  sizeof(senprob_t));
	} else {
		s->pdf =
		    (senprob_t ***) ckd_calloc_3d(s->n_feat, s->n_cw,
						  s->n_sen,
						  sizeof(senprob_t));
	}
	...
	for (i = 0; i < s->n_sen; i++) {
		for (f = 0; f < s->n_feat; f++) {
			if (bio_fread
			    ((void *) pdf, sizeof(float32), s->n_cw, fp,
			     byteswap, &chksum) != s->n_cw) {
				E_FATAL
			...
			for (c = 0; c < s->n_cw; c++) {
				p = -(logs3(pdf[c])); // Note: it takes negative of the log value here
...
senone_t *senone_init(char *mixwfile, char *sen2mgau_map_file,
		      float32 mixwfloor)
{
	senone_t *s;
	int32 n, i;
...
senone_eval(senone_t * s, s3senid_t id, gauden_dist_t ** dist, int32 n_top)
{
	...
	for (f = 0; f < s->n_feat; f++) { // Comment by fang: for s1_39x, n_feat = 1
		fdist = dist[f];

		/* Top codeword for feature f */
		// Comment by fang: refer to senone_mixw_read() in senone.c
		// s->[][][] is a processed mixture weight
		...
		for (t = 1; t < n_top; t++) {
			fwscr = (s->n_gauden > 1) ? fdist[t].dist - (s->pdf[id][f][fdist[t].id] << s-> shift) 
