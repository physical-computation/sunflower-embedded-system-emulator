...
mllr_read_regmat(const char *regmatfile,
		 float32 **** A,
		 float32 *** B, int32 * streamlen, int32 n_stream)
{
	int32 i, j, k, n;
	FILE *fp;
	float32 ***lA, **lB;
	...
	for (i = 0; i < n_stream; ++i) {
		lA[i] =
		    (float32 **) ckd_calloc_2d(streamlen[i], streamlen[i],
					       sizeof(float32));
		lB[i] =
		    (float32 *) ckd_calloc(streamlen[i], sizeof(float32));
	}
	       ...
	       float32 ** B, int32 * streamlen, int32 n_stream)
{
	...
	for (s = 0; s < n_stream; s++) {
		temp =
		    (float64 *) ckd_calloc(streamlen[s], sizeof(float64));
