 ...
 * 
 * HISTORY
 * Jan-16-04 New architecture //Ed 
 * 
 * $Log: fwd.c,v $
 * Revision 1.5  2003/01/29 02:28:49  ffang
 * Add in the code to extract profiling information
 *
 * Revision 1.4  2003/01/28 02:34:33  ffang
 * no message
 *
 * Revision 1.3  2003/01/07 04:59:05  ffang
 * minor bug fix
 *
 * Revision 1.2  2003/01/07 04:22:15  ffang
...
#define LATID2SF(l)	(IS_LATID(lattice[l].history) ? lattice[lattice[l].history].frm + 1 : 0)
	...
	for (b = 0; b < mdef->n_ciphone; b++) {
		...
		for (c1 = 0; c1 < mdef->n_ciphone; c1++) {
			...
			printf("n_pid(%s, %s) = %d\n",
			       ...
			       mdef_ciphone_str(mdef, c1), x[b][c1].n_pid);
		...
		int32 n)
{
	...
	for (i = 0; i < n; i++) {
		if (mdef->phone[p].tmat != mdef->phone[pid[i]].tmat)
			continue;	// Added by Fang Fang: Actually this never happened 

		prevsenmap = mdef->phone[pid[i]].state;
		for (s = 0;
		     (s < n_state - 1) && (senmap[s] == prevsenmap[s]);
		     s++);
...
static void build_lcpid(s3cipid_t b, s3cipid_t rc)
{
	...
	for (lc = 0; lc < mdef->n_ciphone; lc++) {
		p = mdef_phone_id_nearest(mdef, b, lc, rc,
					  WORD_POSN_BEGIN);
...
static void build_lrcpid(s3cipid_t b)
{
	...
	for (lc = 0; lc < mdef->n_ciphone; lc++) {
		lrcpid[b][lc].pid =
		    (s3pid_t *) ckd_calloc(mdef->n_ciphone,
					   sizeof(s3pid_t));
		lrcpid[b][lc].cimap =
		    (s3cipid_t *) ckd_calloc(mdef->n_ciphone,
					     sizeof(s3cipid_t));

		for (rc = 0; rc < mdef->n_ciphone; rc++) {
			lrcpid[b][lc].cimap[rc] = rc;
			lrcpid[b][lc].pid[rc] =
			    mdef_phone_id_nearest(mdef, b, lc, rc,
						  WORD_POSN_SINGLE);
			if ((!mdef->ciphone[b].filler) && word_start_ci[rc]
			    && word_end_ci[lc]
			    && mdef_is_ciphone(mdef,
					       lrcpid[b][lc].pid[rc]))
				n_backoff_ci++;
		}
	...
	for (w = 0; w < dict->n_word; w++) {
		...
		for (l = 1; l < pronlen - 1; l++) {
			...
			if ((!mdef->ciphone[b].filler)
			    && mdef_is_ciphone(mdef, wwpid[w][l]))
				n_backoff_ci++;
...
static void build_xwdpid_map(void)
{
	...
	for (w = 0; w < dict->n_word; w++) {
		...
		word_end_ci[dict->word[w].
			    ciphone[dict->word[w].pronlen - 1]] = 1;
	}
...
static void lattice_dump(FILE * fp)
{
	...
	for (i = 0; i < n_lat_entry; i++) {
		fprintf(fp, "%6d: %5d %6d %11d %s\n", i,
			lattice[i].frm, lattice[i].history,
			lattice[i].score, dict_wordstr(lattice[i].wid));
	}
	fflush(fp);
}
...
static whmm_t *whmm_alloc(int32 pos)
{
	...
	if (!whmm_freelist[k]) {
		...
		whmm_freelist[k] = h =
		    (whmm_t *) ckd_calloc(n, sizeof(whmm_t));
		tmp_scr = (int32 *) ckd_calloc(n_state * n, sizeof(int32));
		tmp_latid =
		    (s3latid_t *) ckd_calloc(n_state * n,
					     sizeof(s3latid_t));
		if (pos == 0)
			tmp_pid =
			    (s3pid_t *) ckd_calloc(n_state * n,
						   sizeof(s3pid_t));
...
static void dump_whmm(s3wid_t w, whmm_t * h, int32 * senscr)
{
	...
	for (s = 0; s < n_state - 1; s++) {
		...
		if (NOT_PID(p))
			...
			printf(" %12d",
			       tmat->tp[mdef->phone[p].tmat][s][s + 1]);
	}
	printf("\n");
	...
	for (s = 0; s < n_state - 2; s++) {
		...
		if (NOT_PID(p))
			...
			printf(" %12d",
			       tmat->tp[mdef->phone[p].tmat][s][s + 2]);
	}
	printf("\n");
		...
		for (to = 0; to < n_state - 1; to++)
			for (from = to + 1; from < n_state - 1; from++)
				if (tmat->tp[i][from][to] > LOGPROB_ZERO)
					E_FATAL
					    ("HMM transition matrix not upper triangular\n");
	}
}
...
static void eval_nonmpx_whmm(s3wid_t w, whmm_t * h, int32 * senscr)
{
	...
	tp = tmat->tp[mdef->phone[p].tmat][0];	// HACK!! Assumes tp 2-D data allocated contiguously 
...
// w: word id
// h: current hmm
// senscr: all senone scores for current frame
{
	s3pid_t pid;
	s3senid_t *sen;
	int32 **tp;
	...
	for (from = n_state - 2; from >= 0; --from) {
		if ((st_sen_scr[from] =
		     h->score[from] + senscr[sen[from]]) < LOGPROB_ZERO)
	...
	for (to = final_state - 1; to >= 0; --to) {
		...
		scr =(tp[to][to] >
		     LOGPROB_ZERO) ? st_sen_scr[to] +
		    tp[to][to] : LOGPROB_ZERO;
...
// senscr: all senone scores for current frame
{
	...
	for (to = final_state - 1; to >= 0; --to) {
		...
		// Comment by fang: there is actually a bug here, the above line should be 
		// bestfrom = to          
		// because if is not modified later, bestfrom state should be itself (self-transition)
		// However,in case of self-transition, the history and pid information need not to be updated
		// therefore, bestfrom = -1 does not cause any problem 
...
static void store_whmmsend2(whmm_t *h, whmm_send2 *h_ws)
{
  h_ws->next=h->next;
  h_ws->score[0]=h->score[0];
  h_ws->score[1]=h->score[1];
  h_ws->score[2]=h->score[2];
  h_ws->score[3]=h->score[3];
  h_ws->history[0]=h->history[0];
  h_ws->history[1]=h->history[1];
  h_ws->history[2]=h->history[2];
  h_ws->history[3]=h->history[3];
  h_ws->pid[0]=h->pid[0];
  h_ws->pid[1]=h->pid[1];
  h_ws->pid[2]=h->pid[2];
  h_ws->pid[3]=h->pid[3];
  h_ws->bestscore=h->bestscore;
  h_ws->pos=h->pos;
  h_ws->rc=h->rc;
  h_ws->active=h->active;
}

static void store_whmmsend1(whmm_t *h, whmm_send1 *h_ws)
{
  h_ws->next=h->next;
  h_ws->score[0]=h->score[0];
  h_ws->score[1]=h->score[1];
...
static int32 whmm_eval(int32 * senscr)
{
	...
	cf = n_frm;
		...
		lock0=get0;
	      ...
	      lock0=1;
              if (h->pos == 0) {
		//store_whmmsend2(h, &h_ws2);
		//emw(x, &h_ws2, senscr, 0);
                eval_mpx_whmm (x, h, senscr);
                n_mpx++;
              } else {
		//store_whmmsend1(h, &h_ws1);
		//enmw(x, &h_ws1, senscr, 1);
                eval_nonmpx_whmm (x, h, senscr);
                n_nonmpx++;
              }
              if (best < h->bestscore)
                best = h->bestscore;
		...
		break;
	      ...
	      lock1=1;
                if (i->pos == 0) {
//		    store_whmmsend2(i, &h_ws2);
		    //emw(y, &h_ws2, senscr, 0);
                    eval_mpx_whmm (y, i, senscr);
                    n_mpx++;
                } else {
//		    store_whmmsend1(h, &h_ws1);
		    //enmw(y, &h_ws1, senscr, 1);
                    eval_nonmpx_whmm (y, i, senscr);
                    n_nonmpx++;
                }
...
//w: word id
//f: frame number
//h: last HMM of this word
{
	s3cipid_t rc, npid;

	if ((n_lat_entry <= 0) || (lattice[n_lat_entry - 1].wid != w)
	    || (lattice[n_lat_entry - 1].frm != f)) {
		/* New lattice entry */
...
//Ed added from word_trans
static void backoff_init()
{
	s3cipid_t rc;
	for (rc = 0; rc < mdef->n_ciphone; rc++) {
...
static void two_word_history(s3latid_t l, s3wid_t * w0, s3wid_t * w1)
{
	...
	//Filter all the filter words before l and get l1
	for (l1 = l; filler_word(lattice[l1].wid);
	     l1 = lattice[l1].history);

/* BHIKSHA HACK - PERMIT MULTIPLE PRONS FOR <s> */
	//Filter all the filter words before l1 and get l0
	if (l1 != -1)
...
/* BHIKSHA HACK - PERMIT MULTIPLE PRONS FOR <s> */
	...
	if (l1 == -1)
		...
		*w0 =
		    (NOT_LATID(l0)) ? BAD_WID : dict_basewid(lattice[l0].
							     wid);
}
...
static void word_enter(s3wid_t w, int32 score, s3latid_t l, s3cipid_t lc)
{
	...
	if (dict->word[w].pronlen > 1) {	/* Multi-phone word; no right context problem */
		...
		// b = dict->word[w].ciphone[0];  // Redundant code
		for (rc = 0, h = whmm[w]; rc < npid; rc++, h = h->next) {
			if (score > h->score[0]) {
//		if (nf<15)
//		fprintf(weptr, "w=%d lc=%d b=%d rc=%d score=%d l=%d\n",w,lc,b, mdef->phone[h->pid[0]].rc, score, l);
				h->score[0] = score;
				h->history[0] = l;
				h->pid[0] = rpid[rc];
				h->active = nf;
...
//	for (l = lat_start; l < n_lat_entry; l++) 
	...
	/*	if (n_word_cand <= 0) {*/
		...
		// Comment by fang 
		// n_word_cand is a static int variable, works when decoding within the words in the given lattice. 
		// here I turned it off
			...
			acc_bowt = 0;
			if ((IS_WID(bw0))
			    && ((n_tg = lm_tglist(bw0, bw1, &tgptr, &bowt))	> 0)) {
			...
			if ((n_bg = lm_bglist(bw1, &bgptr, &bowt)) > 0) {
				...
				for (; n_bg > 0; --n_bg, bgptr++) {
					...
					if (IS_WID(nextwid) && (!tg_trans_done[nextwid]) &&	/* TG transition already done */
...
static void whmm_exit(int32 thresh, int32 wordthresh)
{
	...
	nf = n_frm + 1;
//Ed
	p_lat_entry=frm_latstart[n_frm];
	backoff_init();
	for (w = 0; w < dict->n_word; w++) 
        {
		lock0=get0;
		...
		for (h = whmm[w]; h; h = h->next) {
			// Comment by Fang Fang: Inactive words have already been freed in whmm_eval()
			// So there is no need to check if h->active == n_frm
			...
			if (h->bestscore >= thresh) {
				if (h->pos == pronlen - 1) {
					...
					if (h->score[final_state] +
					    phone_penalty >= thresh) // Default phone_penalty is zero
						whmm_transition(w, h);
				}
...
void fwd_init(void)
{
	...
	if ((NOT_WID(silwid)) || (NOT_WID(startwid))
	    || (NOT_WID(finishwid)))
	...
	if (word_cand_dir) {
		word_cand =
		    (word_cand_t **) ckd_calloc(S3_MAX_FRAMES,
						sizeof(word_cand_t *));
		word_cand_cf =
		    (s3wid_t *) ckd_calloc(dict->n_word + 1,
					   sizeof(s3wid_t));
	}

	/* Space for first lattice entry in each frame (+ terminating sentinel) */
	frm_latstart =
	    (s3latid_t *) ckd_calloc(S3_MAX_FRAMES + 1, sizeof(s3latid_t));
...
*/
	...
	/* Initialize word_ugprob (assuming there is only one LM) */
	{
		...
		word_ugprob =
		    (word_ugprob_t **) ckd_calloc(mdef->n_ciphone,
		...
		for (; n_ug > 0; --n_ug, ugptr++) {
			...
			for (; IS_WID(w); w = dict->word[w].alt) {
				...
				prevwp = NULL;
				for (wp = word_ugprob[ci];
				     wp && (wp->ugprob >= ugprob);
				     wp = wp->next)
					prevwp = wp;
				wp = (word_ugprob_t *)
				    listelem_alloc(sizeof(word_ugprob_t));
				wp->wid = w;
				wp->ugprob = ugprob;
				if (prevwp) {
					wp->next = prevwp->next;
...
static int32 word_cand_load(FILE * fp)
{
	...
	for (i = 0; i < nn; i++) {
		...
		if ((sf < 0) || (sf >= S3_MAX_FRAMES)) {
			E_ERROR
			    ("%s: Startframe error in lattice, line %d: %s\n",
			     uttid, lineno, line);
			return -1;
		}
		...
		if (NOT_WID(w)) {
			E_ERROR
			    ("%s: Unknown word in lattice: %s; ignored\n",
			     uttid, word);
			continue;
		}
		w = dict_basewid(w);
		...
		for (candp = word_cand[sf]; candp && (candp->wid != w);
		     candp = candp->next);
		if (candp)
			continue;

		candp =
		    (word_cand_t *) listelem_alloc(sizeof(word_cand_t));
		candp->wid = w;
...
*/
	...
	if (word_cand_dir) {
		...
		if ((fp = fopen_compchk(str, &ispipe)) == NULL)
			E_ERROR
			    ("fopen_compchk(%s) failed; running full search\n",
			     str);
		else {
			if ((n_word_cand = word_cand_load(fp)) <= 0) {
				E_ERROR
				    ("Bad or empty lattice file: %s; ignored\n",
				     str);
				word_cand_free();
			} else
	...
	for (w = startwid; IS_WID(w); w = dict->word[w].alt)
		word_enter(w, 0, BAD_LATID,
			   dict->word[silwid].ciphone[dict->word[silwid].
						      pronlen - 1]);
	n_frm = 0;
...
//Ed
static void unigram_filler(int32 thresh)
{
	/*
  ...
  // ?? Question, why it doesn't check whether word wp has already accessed from the TG or BG transition
...
//	timing_stop(tm_hmmeval);
	...
	whmm_thresh = bestscr + beam > LOGPROB_ZERO? bestscr + beam:LOGPROB_ZERO ;
	word_thresh = bestscr + wordbeam > LOGPROB_ZERO? bestscr + wordbeam:LOGPROB_ZERO ;
...
//	timing_start(tm_hmmtrans);
	...
	whmm_exit(whmm_thresh, word_thresh); // new_phones is updated in this function
//Ed
    if (frm_latstart[n_frm] < n_lat_entry)
	  unigram_filler(whmm_thresh);
//Ed
...
//Ed
//	if (frm_latstart[n_frm] < n_lat_entry)
//		word_trans(whmm_thresh);	// new_words is updated in this function
//Ed
 ...
 */
static void
lat_seg_ascr_lscr(s3latid_t l, s3wid_t w_rc, int32 * ascr, int32 * lscr)
{
...
static hyp_t *lattice_backtrace(s3latid_t l, s3wid_t w_rc)
{
	...
	if (IS_LATID(l)) {
		prevh =
		    lattice_backtrace(lattice[l].history, lattice[l].wid);
...
{
	printf("[H %6.2fx %2d%%]",
	       tm_hmmeval->t_cpu * 100.0 / n_frm,
	       (int32) (tm_hmmeval->t_cpu * 100.0 / tot));
	printf("[XH %6.2fx %2d%%]", tm_hmmtrans->t_cpu * 100.0 / n_frm,
	       (int32) (tm_hmmtrans->t_cpu * 100.0 / tot));
	printf("[XW %6.2fx %2d%%]", tm_wdtrans->t_cpu * 100.0 / n_frm,
	       (int32) (tm_wdtrans->t_cpu * 100.0 / tot));
}
*/

static void
dag_link(dagnode_t * pd, dagnode_t * d, int32 ascr, int32 ef,
	 daglink_t * byp)
{
	daglink_t *l;
...
static void
dag_update_link(dagnode_t * pd, dagnode_t * d, int32 ascr,
...
int32 dag_build(void)
{
	...
	for (l = 0; l < n_lat_entry; l++) {
		...
		if (!d) {
			d = (dagnode_t *)
			    listelem_alloc(sizeof(dagnode_t));
...
/*
			...
			for (l = frm_latstart[d->sf - 1];
			     l < frm_latstart[d->sf]; l++) {
...
static void dag_add_fudge_edges(int32 fudge, int32 min_ef_range)
{
	...
	for (d = dag.list; d; d = d->alloc_next) {
		...
		for (l = frm_latstart[d->sf]; l < frm_latstart[d->sf + 1];
		     l++) {
			pd = lattice[l].dagnode;	/* Predecessor DAG node */
			...
			if ((pd->wid != finishwid) &&
				...
				lat_seg_ascr_lscr(l, BAD_WID, &ascr,
						  &lscr);
...
static void dag_bestpath(daglink_t * l,	/* Backward link! */
			 ...
			 float64 lwf)
{				/* Language weight multiplication factor */
	dagnode_t *d, *pd;
	daglink_t *pl;
	int32 lscr, score;
	...
	for (pl = d->predlist; pl; pl = pl->next) {
		...
		if (pl->pscr > (int32) 0x80000000) {
			...
			if (pd)
				lscr =
				    lwf *
				    lm_tg_score(dict_basewid(pd->wid),
						dict_basewid(d->wid),
						dict_basewid(src->wid));
			else
				lscr =
				    lwf * lm_bg_score(dict_basewid(d->wid),
						      dict_basewid(src->
								   wid));
			score += lscr;
	...
	printf("%s,%d -> %s,%d = %d\n",
	       ...
	       dict_wordstr(src->wid), src->sf, l->pscr);
#endif
...
hyp_t *dag_search(char *utt)
{
	...
	if (renormalized) {
		E_ERROR
		    ("Scores renormalized during forward Viterbi; cannot run bestpath\n");
		return NULL;
	}
	...
	lwf =
	    f32arg ? ((*f32arg) /
		      *((float32 *) cmd_ln_access("-langwt"))) : 1.0;
