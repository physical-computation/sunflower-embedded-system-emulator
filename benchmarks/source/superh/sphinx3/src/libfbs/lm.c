...
static int bitwidth_ugprob = 32;		/* Added by Fang Fang */
static int bitwidth_ugbowt = 32;		/* Added by Fang Fang */
static int bitwidth_bgprob = 32;		/* Added by Fang Fang */
static int bitwidth_tgprob = 32;		/* Added by Fang Fang */
static int bitwidth_tgbowt = 32;		/* Added by Fang Fang */
...
// Added by Fang Fang
void bitwidth_ugprob_init(int value)
{
	bitwidth_ugprob = value;

}
void bitwidth_ugbowt_init(int value)
{
	bitwidth_ugbowt = value;

}
void bitwidth_bgprob_init(int value)
{
	bitwidth_bgprob = value;
}
void bitwidth_tgprob_init(int value)
{
	bitwidth_tgprob = value;

}
void bitwidth_tgbowt_init(int value)
{
	bitwidth_tgbowt = value;
...
// lm: language model weight, uw: interpolation weight, wip: word insersion penalty
{
	int32 i, loguw, loguw_, p1, p2, loguniform;

	// Added by Fang Fang for debugging 
	/*
	FILE * fp_ugprob, * fp_ugbowt, *fp_bgprob, *fp_tgprob, *fp_tgbowt;
	*/
	float max_ugprob=0, max_ugbowt=0, max_bgprob=0, max_tgprob=0, max_tgbowt=0;
	int mbits_ugprob=32, mbits_ugbowt=32, mbits_bgprob=32, mbits_tgprob=32, mbits_tgbowt=32;
	int quant;
	//end of added code
	...
	// Added by Fang Fang for debugging 
	// fp_ugprob, fp_ugbowt, fp_bgprob, fp_tgprob, fp_tgbowt, 
	/*
	fp_ugprob = fopen ("l_ugprob.fp", "w");
	fp_ugbowt = fopen ("l_ugbowt.fp", "w");
	fp_bgprob = fopen ("l_bgprob.fp", "w");
	fp_tgprob = fopen ("l_tgprob.fp", "w");
	fp_tgbowt = fopen ("l_tgbowt.fp", "w");
	*/
	
	// Find the maximum # of bits for unigram 
	for (i = 0; i < lm->n_ug; i++) {
		if ( -lm->ug[i].prob.f > max_ugprob && lm->ug[i].prob.f!=-99)
			max_ugprob = -lm->ug[i].prob.f ;
		if ( -lm->ug[i].bowt.f > max_ugbowt && lm->ug[i].bowt.f!=-99)
			max_ugbowt = -lm->ug[i].bowt.f ;
	}
	
	// Find the maximum # of bits for bigram 
    for (i = 0; i < lm->n_bgprob; i++){
		if ( -lm->bgprob[i].f > max_bgprob && lm->bgprob[i].f!=-99)
			max_bgprob = -lm->bgprob[i].f ;
	}
	
	// Find the maximum # of bits for trigram 
	for (i = 0; i < lm->n_tgprob; i++){
		if ( -lm->tgprob[i].f > max_tgprob && lm->tgprob[i].f!=-99)
			max_tgprob = -lm->tgprob[i].f ;
	}
	for (i = 0; i < lm->n_tgbowt; i++){
		if ( lm->tgbowt[i].f > max_tgbowt && lm->tgbowt[i].f!=-99)
			max_tgbowt = lm->tgbowt[i].f ;
	}
	
	if(lm->n_ug){	
		int t1, t2;
		t1 = log10_to_logs3(max_ugprob) + loguw;
		t2 = loguniform + loguw_;
		t1 = logs3_add(t1, t2);
		max_ugprob = t1 * lw + lm->wip;	
		mbits_ugprob = ceil(log10(max_ugprob)/log10(2));
	
		max_ugbowt = (log10_to_logs3(max_ugbowt) * lw );
		mbits_ugbowt = ceil(log10(max_ugbowt)/log10(2));
	}
	
	if(lm->n_bgprob){
		max_bgprob = (log10_to_logs3(max_bgprob) * lw + lm->wip);
		mbits_bgprob = ceil(log10(max_bgprob)/log10(2));
	}
	
	if(lm->n_tgprob){
		max_tgprob = (log10_to_logs3(max_tgprob) * lw + lm->wip);
		mbits_tgprob = ceil(log10(max_tgprob)/log10(2));
	}
	
	if(lm->n_tgbowt){
		max_tgbowt = (log10_to_logs3(max_tgbowt) * lw );
		mbits_tgbowt = ceil(log10(max_tgbowt)/log10(2));
	}
	

	/*
	fprintf(fp_ugprob, "MAX is %f\n", max_ugprob);
	fprintf(fp_bgprob, "MAX is %f\n", max_bgprob);
	fprintf(fp_tgprob, "MAX is %f\n", max_tgprob);
	fprintf(fp_ugbowt, "MAX is %f\n", max_ugbowt);
	fprintf(fp_tgbowt, "MAX is %f\n", max_tgbowt);
	...
	fclose(fp_tgbowt);
	*/
	// End of added code by Fang Fang
	
	// Unigram prob
	quant = mbits_ugprob - bitwidth_ugprob;
	...
	for (i = 0; i < lm->n_ug; i++) {
		...
		// quantization 
		if(bitwidth_ugprob < 32 && quant > 0){
			lm->ug[i].prob.l = ((lm->ug[i].prob.l + (1 << (quant-1))) >> quant) << quant;
		}
	}
	
	// Unigram bowt
	quant = mbits_ugbowt - bitwidth_ugbowt;
	
	for (i = 0; i < lm->n_ug; i++) {		
		lm->ug[i].bowt.l = log10_to_logs3(lm->ug[i].bowt.f) * lw;
		// quantization 
		if(bitwidth_ugbowt < 32 && quant > 0){
			lm->ug[i].bowt.l = ((lm->ug[i].bowt.l + (1 << (quant-1))) >> quant) << quant;
		}
	}
	// Bigram prob
	quant = mbits_bgprob - bitwidth_bgprob;
	
	for (i = 0; i < lm->n_bgprob; i++) {
		lm->bgprob[i].l =  log10_to_logs3(lm->bgprob[i].f) * lw + lm->wip;
		
		// quantization 
		if(bitwidth_bgprob < 32 && quant > 0){
			lm->bgprob[i].l = ((lm->bgprob[i].l + (1 << (quant-1))) >> quant)  << quant;
		}
	}
	...
	if (lm->n_tg > 0) {
		// Trigram prob
		quant = mbits_tgprob - bitwidth_tgprob;
		
		for (i = 0; i < lm->n_tgprob; i++) {
			lm->tgprob[i].l =  log10_to_logs3(lm->tgprob[i].f) * lw + lm->wip;
			
			// quantization 
			if(bitwidth_tgprob < 32 && quant > 0){
				lm->tgprob[i].l = ((lm->tgprob[i].l + (1 << (quant-1))) >> quant)  << quant;
			}
		
		}
		// Trigram bowt
		quant = mbits_tgbowt - bitwidth_tgbowt;
		
		for (i = 0; i < lm->n_tgbowt; i++) {
			lm->tgbowt[i].l = log10_to_logs3(lm->tgbowt[i].f) * lw;
			
			// quantization 
			if(bitwidth_tgbowt < 32 && quant > 0){
				lm->tgbowt[i].l = ((lm->tgbowt[i].l+ (1 << (quant-1))) >> quant) << quant;
			}
...
static int32
lm_add(lm_t * lm, char *name, float64 lw, float64 uw, float64 wip)
{
	...
	if (n_lm == n_lm_alloc) {
		if (n_lm_alloc > 0)
			lmset =
			    (lmset_t *) ckd_realloc(lmset,
						    (n_lm +
						     15) *
						    sizeof(lmset_t));
		else
			lmset =
			    (lmset_t *) ckd_calloc(15, sizeof(lmset_t));
		n_lm_alloc += 15;
	}
	lmset[n_lm].lm = lm;
...
static int32 lm_read_dump(char *file, char *name)
{
	...
	if (k == strlen(darpa_hdr) + 1)
		...
		if (k == strlen(darpa_hdr) + 1)
			...
			SWAP_UINT32(k);
			E_FATAL
			    ("Bad magic number: %d(%08x), not an LM dumpfile??\n",
			     k, k);
		}
	}
	...
	if (vn <= 0) {
		...
		if (vn <= -2) {
			...
			if ((k < 1) || (k > 15))
				E_FATAL
				    ("log2(bg_seg_sz) outside range 1..15\n",
				     k);
			lm->log_bg_seg_sz = k;
		} else
	...
	if (lm->n_bg > 0) {
		...
		lm->membg =
		    (membg_t *) ckd_calloc(lm->n_ug, sizeof(membg_t));
	}
	...
	if (lm->n_tg > 0) {
		...
		lm->tginfo =
		    (tginfo_t **) ckd_calloc(lm->n_ug, sizeof(tginfo_t *));
	}
	...
	if (lm->n_bg > 0) {
		...
		lm->bgprob =
		    (lmlog_t *) ckd_calloc(lm->n_bgprob, sizeof(lmlog_t));
		if (fread
		    (lm->bgprob, sizeof(lmlog_t), lm->n_bgprob,
		     lm->fp) != lm->n_bgprob)
			E_FATAL("fread(%s) failed\n", file);
		if (lm->byteswap) {
			for (i = 0; i < lm->n_bgprob; i++)
	...
	if (lm->n_tg > 0) {
		...
		lm->tgbowt =
		    (lmlog_t *) ckd_calloc(lm->n_tgbowt, sizeof(lmlog_t));
		if (fread
		    (lm->tgbowt, sizeof(lmlog_t), lm->n_tgbowt,
		     lm->fp) != lm->n_tgbowt)
			E_FATAL("fread(%s) failed\n", file);
		if (lm->byteswap) {
			for (i = 0; i < lm->n_tgbowt; i++)
		...
		/* Allocate and read trigram prob table */
		lm->tgprob =
		    (lmlog_t *) ckd_calloc(lm->n_tgprob, sizeof(lmlog_t));
		if (fread
		    (lm->tgprob, sizeof(lmlog_t), lm->n_tgprob,
		     lm->fp) != lm->n_tgprob)
			E_FATAL("fread(%s) failed\n", file);
		if (lm->byteswap) {
			for (i = 0; i < lm->n_tgprob; i++)
	...
	// Added by Fang Fang for debugging
	/*dump unigram probability
	   word_prob_fp = fopen ("word_prob.txt", "w");
	   for ( i = 0; i < lm->n_ug; i++){
	   fprintf(word_prob_fp, "%24s  %f\n",  lm->wordstr[i], lm->ug[i].prob.f);
	   }
...
#endif
		dict2lmwid =
		    (s3lmwid_t *) ckd_calloc(dictsize, sizeof(s3lmwid_t));
	}
...
int32 lm_bglist(s3wid_t w1, bg_t ** bgptr, int32 * bowt)
{
	...
	n = (lm->n_bg >
	     0) ? lm->ug[lw1 + 1].firstbg - lm->ug[lw1].firstbg : 0;
...
static void load_tg(lm_t * lm, s3lmwid_t lw1, s3lmwid_t lw2)
{
	...
	if (n > 0) {
		...
		if (fseek(lm->fp, lm->tgoff + t * sizeof(tg_t), SEEK_SET) <
		    0)
			E_FATAL_SYSTEM("fseek failed\n");
...
static int32 sentence_lmscore(char *line)	//#if (NO_DICT)
{
	int32 wid[3];
	char *_lp;
	int32 linelen;
	...
	while (sscanf(_lp, "%s%n", word, &linelen) == 1) {
		...
		if ((wid[2] = lm_wid(word)) < 0) {
			if ((wid[2] = lm_wid("<UNK>")) < 0) {
				fprintf(stderr,
					"%s(%d): Unknown word: %s, skipping sentence\n",
					__FILE__, __LINE__, word);
				return (0);
			} else
				fprintf(stderr,
					"%s(%d): Unknown word: %s, using <UNK>\n",
					__FILE__, __LINE__, word);
		}
...
main(int32 argc, char *argv[])	//#if (NO_DICT)
{
	char line[4096];
	int32 score, k;
