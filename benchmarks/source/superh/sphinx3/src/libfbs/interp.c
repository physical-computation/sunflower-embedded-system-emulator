...
static int32 interp_read(interp_t * ip, char *file_name)
{
	FILE *fp;
	char version[1024];
	...
	/* Read #senones */
	if (bio_fread
	    (&(ip->n_sen), sizeof(int32), 1, fp, byteswap, &chksum) != 1)
	...
	for (i = 0; i < ip->n_sen; i++) {
		if (bio_fread
		    (&f, sizeof(float32), 1, fp, byteswap, &chksum) != 1)
...
interp_t *interp_init(char *file)
{
	interp_t *ip;
	...
	if (interp_read(ip, file) != S3_SUCCESS)
