...
static int tmatwidth = 0; 

void tmatwidth_init(int v){
	tmatwidth = v;
}
...
static int32 tmat_read(tmat_t * t, char *file_name)
{
	...
	if ((bio_fread
	     (&(t->n_tmat), sizeof(int32), 1, fp, byteswap, &chksum) != 1)
	    ||
	    (bio_fread(&row_dim, sizeof(int32), 1, fp, byteswap, &chksum)
	     != 1)
	    ||
	    (bio_fread
	     (&(t->n_state), sizeof(int32), 1, fp, byteswap, &chksum)
	     != 1)
	    || (bio_fread(&i, sizeof(int32), 1, fp, byteswap, &chksum) !=
		1)) {
	...
	t->tp =
	    (int32 ***) ckd_calloc_3d(t->n_tmat, t->n_state - 1,
				      t->n_state, sizeof(int32));

	/* Temporary structure to read in the float data */
	tp = (float32 **) ckd_calloc_2d(t->n_state - 1, t->n_state,
					sizeof(float32));

	/* Read transition matrices, normalize and floor them, and convert to logs3 domain */
	tp_per_tmat = (t->n_state - 1) * t->n_state;

	/* quant is added by Fang to quantize tmat */
	{ int quant = (int) (log10(fabs(logs3(t->tpfloor)))/log10(2.0)+1);
	  quant = (quant > tmatwidth)? quant - tmatwidth:0;
	...
	for (i = 0; i < t->n_tmat; i++) {
		...
		/* Normalize and floor */
			...
			if (vector_normalize(tp[j], t->n_state) !=
			    S3_SUCCESS)
				E_ERROR
				    ("Normalization failed for tmat %d from state %d\n",
				     i, j);
			...
			for (k = 0; k < t->n_state; k++) {
				if(tmatwidth !=0){
					t->tp[i][j][k] =
				    	(tp[j][k] ==
				    	 0.0) ? LOGPROB_ZERO : logs3(tp[j][k]);
			
			  		 if (quant > 0)
						t->tp[i][j][k] = ((t->tp[i][j][k] + (1<< (quant-1)))>> quant ) << quant;     
				}
				else
					t->tp[i][j][k] =
				    	(tp[j][k] ==
	...
	// Added by Fang to check the largest transition probability
/*	{
		int max = LOGPROB_ZERO;
		int min = 0; 
		for (i = 0 ; i < t->n_tmat; i++)
			for (j = 0; j< t->n_state-1; j++)
				for (k = 0; k < t->n_state; k ++){
		...
		printf("FFFFFFFF max tp is %d FFFFFFFFF   min tp is %d\n", max, min);
	}// end			
*/
	ckd_free_2d((void **) tp);
...
int32 tmat_chk_uppertri(tmat_t * tmat)
{
	...
	for (i = 0; i < tmat->n_tmat; i++) {
		for (to = 0; to < tmat->n_state - 1; to++)
			for (from = to + 1; from < tmat->n_state - 1;
			     from++)
				if (tmat->tp[i][from][to] > LOGPROB_ZERO) {
					E_ERROR
					    ("Tmat %d not upper triangular\n",
					     i);
					return -1;
				}
	}
