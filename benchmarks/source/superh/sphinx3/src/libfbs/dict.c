...
static int32 dict_read(FILE * fp)
{
	...
	while (fgets(line, sizeof(line), fp) != NULL) {
		...
		for (np = 0; (plen = nextword(lp, delim, &pron, &tmp)) > 0;
		     np++) {
			lp = pron + plen;
		...
		if (np == 0)
			E_ERROR
			    ("Line %d: No pronunciation for %s; ignored\n",
			     lineno, word);
		else {
			word[wlen] = '\0';
			if (dict_add_word(word, p, np) < 0)
				E_ERROR
				    ("Line %d: dict_add_word (%s) failed; ignored\n",
				     lineno, word);
		}
	}
...
int32 dict_filler_word(s3wid_t w)
{
	...
	return ((w >= d->filler_start)
		&& (w <= d->filler_end)) ? TRUE : FALSE;
}
