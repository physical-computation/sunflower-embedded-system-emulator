...
static void ciphone_add(mdef_t * m, char *ci, s3pid_t p)
{
	...
	temp3 += strlen(mdef->ciphone[p].name);
...
static ph_lc_t *find_ph_lc(ph_lc_t * lclist, s3cipid_t lc)
{
	...
	for (lcptr = lclist; lcptr && (lcptr->lc != lc);
	     lcptr = lcptr->next);
	return lcptr;
}
...
static ph_rc_t *find_ph_rc(ph_rc_t * rclist, s3cipid_t rc)
{
	...
	for (rcptr = rclist; rcptr && (rcptr->rc != rc);
	     rcptr = rcptr->next);
	return rcptr;
}
...
static int32
parse_base_line(mdef_t * m, char *line, int32 lineno, s3pid_t p)
{
	int32 wlen, n;
	char *word;
	s3cipid_t ci;
	...
	ci = mdef_ciphone_id(m, word);	// if word is not in the ciphone, return -1
	// else return the id
	if (IS_CIPID(ci))
	...
	for (n = 0; n < 3; n++) {
		...
		if ((wlen != 1) || (word[0] != '-'))
			E_FATAL
			    ("Line %d: %s instead of '-' in base phone line\n",
			     word, lineno);
		line = word + wlen;
		word[wlen] = tmp;
	}
	...
	if (triphone_add
	    (m, ci, BAD_CIPID, BAD_CIPID, WORD_POSN_UNDEFINED, p) < 0)
...
static int32
parse_tri_line(mdef_t * m, char *line, int32 lineno, s3pid_t p)
{
	int32 wlen;
	char *word;
	s3cipid_t ci, lc, rc;
	...
	if (((wlen = nextword(line, delim, &word, &tmp)) < 0)
	    || (word[1] != '\0'))
		E_FATAL
		    ("Line %d: Missing or bad triphone word-position spec\n",
		     lineno);
	switch (word[0]) {
	case 'b':
	...
	case 'i':
		...
		break;
	default:
		E_FATAL("Line %d: Bad word-position spec: %s\n", lineno,
			word);
	}
	line = word + wlen;
	word[wlen] = tmp;
...
static int32
noncomment_line(char *line, int32 size, int32 * n_read, FILE * fp)
{
...
mdef_t *mdef_init(char *mdeffile)
{
	...
	if (strncmp(buf, MODEL_DEF_VERSION, strlen(MODEL_DEF_VERSION)) !=
	    0)
		E_FATAL
		    ("Line %d: Version error.  Expecing %s, but read %s\n",
	...
	senmap =
	    (s3senid_t *) ckd_calloc(max_phone * mdef->n_emit_state,
				     sizeof(s3senid_t));
	...
	mdef->cd2cisen =
	    (s3senid_t *) ckd_calloc(mdef->n_sen, sizeof(s3senid_t));
	...
	E_INFO
	    ("%d CI-phones, %d CD-phones, %d emitting states/phone, %d sen, %d CI-sen\n",
	     mdef->n_ciphone, mdef->n_phone - mdef->n_ciphone,
	     mdef->n_emit_state, mdef->n_sen, mdef->n_ci_sen);
