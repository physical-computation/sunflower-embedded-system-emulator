 ...
 * Edges are formed if permitted by time adjacency.  (See comment before dag_load.)
 */
typedef struct dagnode_s {
    ...
    struct daglink_s *predlist;		/* List of successor nodes (adjacent in time) */
} dagnode_t;

/* A DAG node can have several successor or predecessor nodes, each represented by a link */
typedef struct daglink_s {
    dagnode_t *node;		/* Target node (source determined by dagnode_t.succlist) */
    struct daglink_s *next;	/* Next in same dagnode_t.succlist */
    int32 ascr;			/* Acoustic score for segment of source node ending just
				   before the end point of this link */
    int32 hscr;			/* Heuristic score from end of link to dag exit node */
    int32 bypass;		/* Whether this is a filler bypass link */
} daglink_t;
...
typedef struct {
    ...
    int32 nfrm;
    int32 nlink;		/* Total #links, including bypass */
    int32 nbypass;		/* #bypass links */
} dag_t;
static dag_t dag;
...
static int32 dag_bypass_filler_nodes ( void )
{
	...
	for (plink = d->predlist; plink; plink = plink->next) {
	    pnode = plink->node;
	    scr = plink->ascr + fillpen(dict_basewid (d->wid));
...
static void dag_compute_hscr ( void )
{
    ...
    for (d = dag.list; d; d = d->alloc_next) {
	bw0 = filler_word (d->wid) ? BAD_WID : dict_basewid (d->wid);
	...
	for (l1 = d->succlist; l1; l1 = l1->next) {
		...
		bw1 = filler_word (d1->wid) ? BAD_WID : dict_basewid (d1->wid);
		if (NOT_WID(bw1)) {
		    bw1 = bw0;
		    bw0 = BAD_WID;
		}
		...
		for (l2 = d1->succlist; l2; l2 = l2->next) {
		    ...
		    bw2 = dict_basewid (d2->wid);
		    hscr = l2->hscr + l2->ascr + lm_tg_score (bw0, bw1, bw2);
...
#if 0
static void heap_dump (heap_t *top, int32 level)
{
    int32 i;
    ...
    printf ("%s %d %d %d %d\n", dict_wordstr(top->nb->dagnode->wid),
 ...
 */
static heap_t *heap_insert (heap_t *root, ppath_t *new)
{
    heap_t *h;
    ppath_t *pp;
    
    if (! root) {
	h = (heap_t *) listelem_alloc (sizeof(heap_t));
	h->ppath = new;
	h->left = h->right = NULL;
	h->nl = h->nr = 0;
	return h;
    ...
    if (root->nl > root->nr) {
	root->right = heap_insert (root->right, new);
	root->nr++;
    } else {
	root->left = heap_insert (root->left, new);
	root->nl++;
    }
 ...
 */
static heap_t *heap_pop (heap_t *root)
{
    heap_t *l, *r;
    ...
    if (! l) {
	if (! r) {
	    listelem_free ((char *) root, sizeof(heap_t));
	    return NULL;
	} else {
	    root->ppath = r->ppath;
	    root->right = heap_pop (r);
	    root->nr--;
	}
    } else {
	if ((! r) || (l->ppath->tscr >= r->ppath->tscr)) {
	    root->ppath = l->ppath;
	    root->left = heap_pop (l);
	    root->nl--;
	} else {
	    root->ppath = r->ppath;
	    root->right = heap_pop (r);
			...
			uint32 hval, int32 pscr)
{
	...
	for (h1 = hlist->lmhist, h2 = lmhist; h1 && h2; h1 = h1->lmhist, h2 = h2->lmhist) {
		...
		(dict_basewid (h1->dagnode->wid) != dict_basewid (h2->dagnode->wid)))
		break;
	}
...
static void ppath_insert (ppath_t *top, daglink_t *l, int32 pscr, int32 tscr, int32 lscr)
{
    ...
    h = lmhist->histhash - w + dict_basewid (w);
    ...
    heap_root = heap_insert (heap_root, pp);
...
static void ppath_seg_write (FILE *fp, ppath_t *pp, int32 ascr)
{
    ...
    lscr_base = pp->hist ? lm_rawscore (pp->lscr, 1.0) : 0;

    fprintf (fp, " %d %d %d %s",
	     pp->dagnode->sf, ascr, lscr_base, dict_wordstr (pp->dagnode->wid));
}
...
static void nbest_hyp_write (FILE *fp, ppath_t *top, int32 pscr, int32 nfr)
{
	...
	if (pp->hist)
	    lscr_base += lm_rawscore (pp->lscr, 1.0);
	else
	    assert (pp->lscr == 0);
    }
...
static int32 ppath_dump (ppath_t *p)
{
    printf ("PPATH:\n");
    for (; p; p = p->hist) {
	printf ("pscr= %11d, lscr= %9d, tscr= %11d, hash= %11u, pruned= %d, sf= %5d, %s\n",
		p->pscr, p->lscr, p->tscr, p->histhash, p->pruned,
		p->dagnode->sf, dict_wordstr (p->dagnode->wid));
    }
}


int32 nbest_search (char *filename, char *uttid)
{
    FILE *fp;
    float32 f32arg;
    float64 f64arg;
	...
	if (pp) {
	    bw1 = dict_basewid(pp->dagnode->wid);
	    pp = pp->lmhist;
	    bw0 = pp ? dict_basewid(pp->dagnode->wid) : BAD_WID;
	} else
	    bw0 = bw1 = BAD_WID;
	...
	for (l = d->succlist; l; l = l->next) {
	    ...
	    bw2 = dict_basewid (l->node->wid);
	    lscr = (filler_word (bw2)) ? fillpen(bw2) : lm_tg_score (bw0, bw1, bw2);
