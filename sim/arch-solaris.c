...
mlog(State *S, char *fmt, ...)
{
	...
	buf = mcalloc(1, MAX_MIO_BUFSZ, "(char *)buf in arch-solaris.c");
	if (buf == NULL)
	{
	...
	mfree(buf, "(char *)buf in arch-solaris.c");
...
mprint(State *S, int out, char *fmt, ...)
{
	...
	if (buf == NULL)
	{
		mexit("Could not allocate memory for (char *)buf in arch-solaris.c", -1);
	}
