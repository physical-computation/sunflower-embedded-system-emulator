...
mgetpwd(void)
{
	...
	/*	On Linux (libc4, libc5, glibc), let getcwd() auto 	*/
	/*	allocate space for buf. The buffer will be freed by 	*/
	/*	caller, e.g., sf.y.					*/
	/*								*/
...
mlog(State *S, char *fmt, ...)
{
	...
	buf = mcalloc(1, MAX_MIO_BUFSZ, "(char *)buf in arch-linux.c");
	if (buf == NULL)
	{
		mexit("Could not allocate memory for (char *)buf in arch-linux.c", -1);
	}
	...
	mfree(buf, "(char *)buf in arch-linux.c");
